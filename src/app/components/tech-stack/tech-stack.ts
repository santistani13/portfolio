...
const TECHS_BY_CATEGORY: TechIcon[][] = [
  [
...
export class TechStackComponent implements AfterViewInit {
  readonly t = computed(() => translations[this.lang.lang()].techStack);

  readonly categories = computed<TechCategory[]>(() =>
    this.t().categories.map((title, i) => ({
      title,
      emoji: EMOJI[i],
      techs: TECHS_BY_CATEGORY[i],
    }))
  );
